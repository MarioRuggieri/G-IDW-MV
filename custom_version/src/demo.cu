...
int main(int argc, char **argv)
{
    ...
    for (int j=0; j<N_ITER; j++)
    {
        ...
        parallelIDW<<<nBlocks,nThreadsForBlock,shMemSize>>>(devKP_blas, devQP, devW, devKv, devZV, KN, QN, stride, nIter);
    ...
    */
    ...
    free(knownPoints); free(knownPoints_blas); free(queryPoints); free(zValues); free(zValuesGPU); free(knownValues); //free(res);
    /*cudaFree(devKP);*/ cudaFree(devKP_blas); cudaFree(devQP); cudaFree(devZV); cudaFree(devW); cudaFree(devKv); //cudaFree(devRes);
